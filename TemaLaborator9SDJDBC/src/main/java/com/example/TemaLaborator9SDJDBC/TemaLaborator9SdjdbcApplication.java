...
public class TemaLaborator9SdjdbcApplication implements CommandLineRunner {

	private final Logger logger = (Logger) LoggerFactory.getLogger(TemaLaborator9SdjdbcApplication.class);
	...
	public void masiniSub100_000km(){
		masinaJDBCdao.findAll().stream().filter(m->m.getNumar_km()<100000).forEach(System.out::println);
		masinaJDBCdao.findAll().stream().filter(m->m.getNumar_km()<100000).forEach(m->logger.info("{}", m));
	}
	...
	public static void main(String[] args) {
		SpringApplication.run(TemaLaborator9SdjdbcApplication.class, args);
	}
